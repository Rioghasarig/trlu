...
900 inform = 0
    ...
    ! Reuse "mrank" to prevent 'nrank' from being overwritten 
...
981 inform = 10
    go to 990
    ! Not enough space to save the Schur complement
982 inform = 11
    go to 990
...
990 return
  ...
  subroutine lu7elm( m, n, jelm, v, lena, luparm, parmlu,     &
    ...
    integer(ip),   intent(in)    :: q(n)   ! not used
    real(rp),      intent(in)    :: v(m)
    integer(ip),   intent(inout) :: luparm(30), lenU, lenL, lrow,       &
    ...
    integer(ip)            :: i, ilast, imax, k, kmax, l, l1, l2, lmax, &
                              minfre, nfree, nrank1,jw,lw,lw1,lw2,lenw
    ...
    do l = l1, l2
       a(l)    = - a(l) / vmax
       indr(l) =   imax
       call lu7asv(m ,n ,indc(l), lenw,lw1,lw2,l,a(l), &
              lena, luparm, parmlu, lenU, lrow,   &
