 ...
 *
 * Code generation for function '_coder_mcarrqr_api'
 *
 */
...
static real_T (*g_emlrt_marshallIn(const emlrtStack *sp, const mxArray *tau,
  const char_T *identifier))[10];
  ...
  const char_T *identifier))[10]
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = h_emlrt_marshallIn(sp, emlrtAlias(tau), &thisId);
  ...
  const emlrtMsgIdentifier *msgId))[10]
{
  ...
  void mcarrqr_api(const mxArray *prhs[8])
{
  int32_T m;
  int32_T n;
  int32_T blkMax;
  ...
  A = c_emlrt_marshallIn(&st, emlrtAlias(prhs[3]), "A");
  lda = emlrt_marshallIn(&st, emlrtAliasP(prhs[4]), "lda");
  jpvt = e_emlrt_marshallIn(&st, emlrtAlias(prhs[5]), "jpvt");
  tau = g_emlrt_marshallIn(&st, emlrtAlias(prhs[6]), "tau");
  info = emlrt_marshallIn(&st, emlrtAliasP(prhs[7]), "info");

  /* Invoke the target function */
  mcarrqr(&st, m, n, blkMax, *A, lda, *jpvt, *tau, info);
}

/* End of code generation (_coder_mcarrqr_api.c) */
